import {
  ...
  import { getStudentIdOfPFE } from "../../service/stagePfe";
  ...
  import * as apis from "../../service/cv";
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const result = await getStudentIdOfPFE(id);
        const etudiantId = result.studentId;
        console.log(etudiantId);
        setEtudiantData(result);
        const result1 = await apis.getCvbyiduser(etudiantId);
        setCvData(result1);
      } catch (error) {
        console.log(error);
      }
